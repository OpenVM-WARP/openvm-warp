...
template <size_t WIDTH, typename PoseidonParams>
__global__ void cukernel_poseidon2_multibus_tracegen(
    Fp *d_trace,
    size_t trace_height,
    size_t trace_width,
    Fp *d_records,
    ...
    using Poseidon2Row = poseidon2::Poseidon2Row<
        ...
        PoseidonParams::PARTIAL_ROUNDS>;
#ifdef CUDA_DEBUG
    assert(Poseidon2Row::get_total_size() + 2 * num_bus_groups == trace_width);
#endif
    if (idx < trace_height) {
        Poseidon2Row row(d_trace + idx, trace_height);
        if (idx < num_records) {
        ...
        for (size_t group = 0; group < num_bus_groups; ++group) {
            size_t count_offset = 2 * (idx * num_bus_groups + group);
            d_trace[idx + (Poseidon2Row::get_total_size() + 2 * group) * trace_height] =
                idx < num_records ? d_counts[count_offset] : 0;
            d_trace[idx + (Poseidon2Row::get_total_size() + 2 * group + 1) * trace_height] =
                idx < num_records ? d_counts[count_offset + 1] : 0;
        }
    }
}

extern "C" int _poseidon2_multibus_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    Fp *d_records,
    ...
    switch (sbox_regs) {
    case 1:
        cukernel_poseidon2_multibus_tracegen<16, Poseidon2ParamsS1><<<grid, block, 0, stream>>>(
            d_trace,
            height,
            width,
            d_records,
        ...
        break;
    case 0:
        cukernel_poseidon2_multibus_tracegen<16, Poseidon2ParamsS0><<<grid, block, 0, stream>>>(
            d_trace,
            height,
            width,
            d_records,
