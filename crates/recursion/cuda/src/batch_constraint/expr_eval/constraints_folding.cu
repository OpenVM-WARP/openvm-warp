...
__global__ void constraints_folding_tracegen_dynamic(
    Fp *trace,
    size_t height,
    const uint2 *__restrict__ proof_and_sort_idxs,              // [num_valid_rows]
    const AffineFpExt *__restrict__ cur_sum_evals,              // [num_valid_rows]
    const FpExt *__restrict__ values,                           // [num_valid_rows]
    const uint32_t *__restrict__ row_bounds,                    // [num_proofs]
    uint32_t *const *__restrict__ constraint_bounds,            // [num_proofs][num_airs]
...
extern "C" int _constraints_folding_tracegen(
    ...
    if (num_proofs <= 8) {
        ...
        DeviceArrayCopy<uint32_t *> constraint_bounds(d_constraint_bounds, num_proofs, stream);
        DeviceArrayCopy<TraceHeight *> sorted_trace_vdata(
            d_sorted_trace_vdata, num_proofs, stream
        );
        DeviceArrayCopy<FpExt *> eq_ns_device(eq_ns, num_proofs, stream);
        if (row_bounds.status() != cudaSuccess) return row_bounds.status();
        if (constraint_bounds.status() != cudaSuccess) return constraint_bounds.status();
        if (sorted_trace_vdata.status() != cudaSuccess) return sorted_trace_vdata.status();
        if (eq_ns_device.status() != cudaSuccess) return eq_ns_device.status();
        ...
        if (ret) return ret;
        constraints_folding_tracegen_dynamic<<<grid, block, 0, stream>>>(
            d_trace,
            height,
            d_proof_and_sort_idxs,
            d_cur_sum_evals,
