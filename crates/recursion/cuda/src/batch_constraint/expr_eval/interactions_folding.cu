...
__global__ void interactions_folding_tracegen_dynamic(
    Fp *trace,
    size_t height,
    uint2 *__restrict__ idx_keys,                                // [num_valid_rows]
    const AffineFpExt *__restrict__ cur_sum_evals,               // [num_valid_rows]
    const FpExt *__restrict__ values,                            // [num_valid_rows]
    uint32_t *const *__restrict__ air_interaction_bounds,        // [num_proofs][num_airs]
    TraceHeight *const *__restrict__ sorted_trace_vdata,         // [num_proofs][num_airs]
    InteractionRecord *const *__restrict__ records,              // [num_proofs][num_interactions]
    FpExt *const *__restrict__ xis,                              // [num_proofs][l_skip + n_global]
    const FpExtWithTidx *per_proof,                              // [num_proofs]
    const uint32_t *__restrict__ num_airs,                       // [num_proofs]
    const uint32_t *__restrict__ n_logups,                       // [num_proofs]
    uint32_t num_valid_rows,
    uint32_t l_skip
) {
    uint32_t global_row_idx = blockIdx.x * blockDim.x + threadIdx.x;
...
extern "C" int _interactions_folding_tracegen(
    ...
    if (num_proofs <= 8) {
        ...
        DeviceArrayCopy<uint32_t *> air_interaction_bounds(
            d_air_interaction_bounds, num_proofs, stream
        );
        DeviceArrayCopy<uint32_t *> interaction_row_bounds(
            d_interaction_row_bounds, num_proofs, stream
        );
        DeviceArrayCopy<TraceHeight *> sorted_trace_vdata(
            d_sorted_trace_vdata, num_proofs, stream
        );
        DeviceArrayCopy<InteractionRecord *> records(d_records, num_proofs, stream);
        DeviceArrayCopy<FpExt *> xis(d_xis, num_proofs, stream);
        DeviceArrayCopy<uint32_t> num_airs_device(h_num_airs, num_proofs, stream);
        DeviceArrayCopy<uint32_t> n_logups_device(h_n_logups, num_proofs, stream);
        if (row_bounds.status() != cudaSuccess) return row_bounds.status();
        if (air_interaction_bounds.status() != cudaSuccess) return air_interaction_bounds.status();
        if (interaction_row_bounds.status() != cudaSuccess) return interaction_row_bounds.status();
        if (sorted_trace_vdata.status() != cudaSuccess) return sorted_trace_vdata.status();
        if (records.status() != cudaSuccess) return records.status();
        if (xis.status() != cudaSuccess) return xis.status();
        if (num_airs_device.status() != cudaSuccess) return num_airs_device.status();
        if (n_logups_device.status() != cudaSuccess) return n_logups_device.status();
        ret = cudaStreamSynchronize(stream);
        if (ret) return ret;

        reverse_affines_setup_dynamic<<<grid, block, 0, stream>>>(
            d_idx_keys,
            d_cur_sum_evals,
            d_per_proof,
            d_values,
            row_bounds.get(),
            interaction_row_bounds.get(),
            num_valid_rows
        );
        ret = CHECK_KERNEL();
        if (ret) return ret;
        ...
        if (ret) return ret;
        interactions_folding_tracegen_dynamic<<<grid, block, 0, stream>>>(
            d_trace,
            height,
            d_idx_keys,
            d_cur_sum_evals,
            d_values,
            air_interaction_bounds.get(),
